...
var ViewManager = exports.ViewManager = function () {
    ...
    _createClass(ViewManager, [{
        ...
        value: function onHashChange() {
            ...
            this.router.updateState(state, _jquery2['default'].url().attr('relative'), document.title);
        }
    }, {
        key: 'getActiveView',
        ...
        value: function isInternalHost(host) {
            ...
            var domainRe = new RegExp('^(?:[\\w-\\.]+\\.)?(' + _jquery2['default'].url().attr('host') + domains + ')$');
        ...
        value: function isLinkExternal(element) {
            var $currentUrl = _jquery2['default'].url(),
                hostName = $currentUrl.attr('host'),
                path = $currentUrl.attr('path'),
                $url = _jquery2['default'].url(element.href),
        ...
        value: function shouldHandleLink(element) {
            ...
            // Do not handle files.
            var $url = _jquery2['default'].url(element.href);
            if ($url.attr('file')) {
                return false;
            }
