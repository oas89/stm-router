...
var Router = exports.Router = function () {
    function Router(options) {
        ...
        (0, _jquery2['default'])(document).ready(function () {
            ...
            (0, _jquery2['default'])(window).on('popstate.router', this.onPopState.bind(this));
        }.bind(this));
    }
    ...
    _createClass(Router, [{
        ...
        value: function start() {
            ...
            if (!history.state) {
                ...
                var url = _jquery2['default'].url().attr('relative');
                this.updateState(state, url, document.title);
            }
            this._route(_jquery2['default'].url().attr('relative'), (0, _jquery2['default'])('html').html(), { from: 'initial' });
        }
        ...
        value: function _route(url, data, options, view) {
            var state, handler;
            options = _jquery2['default'].extend({ shouldNavigate: true }, options);
        ...
        value: function _load(url, options) {
            ...
            this._xhr = _jquery2['default'].get(url, data).done(function (data, textStatus, jqXHR) {
                this._xhr = null;
        ...
        value: function handleURL(url, options) {
            if (!this.supportsPushState) {
                ...
                this._fallback(_jquery2['default'].extend({ url: url }, options));
                return;
            }
            if (!options) {
                options = { data: [] };
            ...
            var _options = _jquery2['default'].extend({}, { from: 'external' }, options);
        ...
        value: function trigger(events, params) {
            (0, _jquery2['default'])(this).trigger(events, params);
        }
    }, {
        key: '_fallback',
        value: function _fallback(options) {
            var url = _jquery2['default'].isFunction(options.url) ? options.url() : options.url,
            ...
            if (method !== 'GET' && method !== 'POST') {
                form.append((0, _jquery2['default'])('<input>', {
                    type: 'hidden',
                    name: '_method',
            ...
            if (typeof data === 'string') {
                _jquery2['default'].each(data.split('&'), function (index, value) {
                    var pair = value.split('=');
                    form.append((0, _jquery2['default'])('<input>', { type: 'hidden', name: pair[0], value: pair[1] }));
                });
            } else if (_jquery2['default'].isArray(data)) {
                for (var i = 0; i < data.length; i++) {
                    if (data[i].value) {
                        form.append((0, _jquery2['default'])('<input>', _jquery2['default'].extend({ type: 'hidden' }, data[i])));
                    }
                }
            } else if ((typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object') {
                for (var key in data) {
                    if (data.hasOwnProperty(key)) {
                        form.append((0, _jquery2['default'])('<input>', { type: 'hidden', name: key, value: data[key] }));
                    }
                }
            }
